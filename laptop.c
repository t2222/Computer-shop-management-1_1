...
{
printf("\n\tTotal price of %d pieces %d no Laptop (%s) : %ld\n\t\tConfirm order ? Press [y] or [Y] for YES and [n] for no :",q+1,ch,user.laptops[ch],q*user.lap_price[ch]) ;
}
void laptop()
{
    ...
    system("color 9F");
    puts("\n\t\t\t\tLaptops");
    printf("\n\n\t\tEnter laptop budget below (highest amounnt). N.B: enter 0 0 for no budget\n\n\t\t\t\tBudget = ");
	...
	if (j==0){
		system("cls") ;
		printf("\n\n\t\t\tSORRY , NO ITEAM ON YOUR BUDGET . RE-ENTER YOUR BUDGET AGAIN.") ;
		laptop() ;
	}
	...
	printf("\n\t\tHow many ? : ") ;
	scanf("%d",&q) ; // q for quantity
	//see if input quantity is valid
	if (q>user.lap_stock[ch])
	{
		system("cls") ;
		printf("\n\t\tInvalid input. Input must be equal to or less than %d.\n",user.lap_stock[ch]) ;
		laptop() ;
	}
...
//show bought products after deleting
	...
	printf("\n\n\t\tTotal cost is :%ld taka\nPress any key to go to main menu.",user.total) ;
	getchar() ;
	printf("\n\t\t<<Writing to bs.txt FILE>>") ;
	
	{
		//Writing to FILE bx.txt	
		b = fopen ("bs.txt","a+") ;
		//show bought products after deleting
		int s = 0 ; //for serial
		for (i = 0 ; i < x ; i++){
	...
	printf("\n\t\tTotal cost is :%ld taka . Press any key to go to main menu .",user.total) ;
	getchar() ;
	//write to file
	printf("\n\t\t<<Writing to bs.txt FILE>>") ;
	
	//begin to write to bs.txt
